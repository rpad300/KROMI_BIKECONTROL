  ...
  return (
    <div className="bg-gray-800 rounded-xl p-2">
      <div className="grid grid-cols-4 gap-1 text-center">
        <MCell value={power > 0 ? String(power) : '0'} label="PWR" unit="W" color={power > 200 ? 'text-orange-400' : 'text-white'} />
        <MCell value={torque > 0 ? torque.toFixed(1) : '0'} label="TRQ" unit="Nm" />
        <MCell value={cadence > 0 ? String(cadence) : '0'} label="CAD" unit="rpm" />
        <MCell value={current > 0 ? current.toFixed(1) : '0'} label="CUR" unit="A" color={current > 5 ? 'text-yellow-400' : 'text-white'} />
      </div>
      ...
      {hasGear && (
        <div className="flex items-center justify-between mt-1.5 pt-1.5 border-t border-gray-700">
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-gray-500">GEAR</span>
            {frontGear > 0 && (
              <span className="text-sm font-bold text-gray-400">{frontGear}×</span>
            )}
          ...
          <div className="flex gap-0.5 items-end h-4">
            {Array.from({ length: 12 }, (_, i) => i + 1).map((g) => (
              <div
                key={g}
                className={`w-1 rounded-sm ${g === rearGear ? 'bg-blue-400' : 'bg-gray-700'}`}
...
function MCell({ value, label, unit, color }: {
  ...
  return (
    <div>
      ...
      <div className="text-[9px] text-gray-500">{label} <span className="text-gray-600">{unit}</span></div>
    </div>
  );
}
